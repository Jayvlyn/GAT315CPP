...
Vector2 jlScreenSize = { 1280, 720 }; // Screen resolution in pixels
float jlScreenZoom = 1.0f; // Zoom level for the view
float jlViewSize = 25.0f; // Size of the view in world units
...
Vector2 ConvertScreenToWorld(Vector2 screen)
{
    ...
    float ratio = jlScreenSize.x / jlScreenSize.y;

    // Calculate the extents of the view based on the view size and aspect ratio
    Vector2 extents = (Vector2){ jlViewSize * ratio, jlViewSize };
    extents = Vector2Scale(extents, jlScreenZoom); // Scale the extents by the zoom level
    ...
    float nx = screen.x / jlScreenSize.x;
    float ny = (jlScreenSize.y - screen.y) / jlScreenSize.y;
...
Vector2 ConvertWorldToScreen(Vector2 world)
{
    ...
    Vector2 screen;
    screen.x = nx * jlScreenSize.x;
    screen.y = (1.0f - ny) * jlScreenSize.y;
