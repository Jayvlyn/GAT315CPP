...
bool Intersects(jlBody* body1, jlBody* body2)
{
...
{
	for (jlBody* body1 = bodies; body1; body1 = body1->next)
	{
		for (jlBody* body2 = body1->next; body2; body2 = body2->next)
		{
			...
			{
				jlContact_t* contact = GenerateContact(body1, body2);
...
{
	jlContact_t* contact = (jlContact_t*)malloc(sizeof(jlContact_t));
	assert(contact);

	memset(contact, 0, sizeof(jlContact_t));
